...
//    }
    ...
    public static List<Object1> getJSONArray(String path) {
        String json;
        ...
        try {
            if (conn.getResponseCode() == 200) {
                ...
                JSONArray jsonArray = new JSONArray(json);
                for(int i =0;i<jsonArray.length();i++){
                    list.add(fromJson(jsonArray.get(i).toString(),Object1.class));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * @return 返回 List
     */
    public static List<Object2> getJSONObject(String path){
        ...
        try {
            if (conn.getResponseCode() == 200) {
                ...
                object2.setSuccess(success);
                object2.setTotal(total);
                // json对象中有一个数组数据，又可以使用getJSONArray获取数组
                JSONArray jsonArray = jsonObject.getJSONArray("appList");
                List<Object2.App> appList = new ArrayList<>();
                for (int i = 0; i < jsonArray.length(); i++) {
                    JSONObject item = jsonArray.getJSONObject(i);
                    Object2.App app = fromJson(item.toString(),Object2.App.class);
                    appList.add(app);
                }
                object2.setAppList(appList);
                list.add(object2);
            }
        } catch (IOException e) {
            e.printStackTrace();
